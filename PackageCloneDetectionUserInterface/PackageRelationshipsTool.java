...
public class PackageRelationshipsTool extends Tool
{
        JTabbedPane m_Tab;
	...
	public void GenerateCallGraph()
	{
		...
		try {
	                ...
	                FileReader readFile = null;
               	        ...
               	        readFile = new FileReader("PackageRelationshipsGraph.xml");
...
*/
			m_ProgramGraphBuilder.ParsePackageRelationshipsResults(reader);

               	        readFile = new FileReader("PackageRelationshipsGraph.xml");
               	        reader = new BufferedReader(readFile);
			m_Overview.GenerateOverview(reader);

		} catch (Exception e) {
			System.out.format("exception %s", e.getMessage());
		}
		m_Overview.m_OverviewGraphComponent.repaint();
	}

	public PackageRelationshipsTool(ApplicationFrame applicationFrame, String distro)
	{
...
//		m_Graph.setMinimumGraphSize(new mxRectangle(0.0, 0.0, 800.0, 800.0));
		m_Layout = new mxFastOrganicLayout(m_Graph);
//		m_Layout = new mxCircleLayout(m_Graph);
//		m_Layout = new mxPartitionLayout(m_Graph);
//		m_Layout = new mxHierarchicalLayout(m_Graph);
//		m_Layout = new mxCompactTreeLayout(m_Graph);
		...
		m_Frame = new JInternalFrame("Package Relationships", true, true);
		m_Frame.setSize(1150, 800);
		m_Frame.setResizable(true);
		m_Frame.setBackground(new Color(242, 242, 242));

		m_ButtonPanel = new PackageRelationshipsToolBar(m_ApplicationFrame, this);
